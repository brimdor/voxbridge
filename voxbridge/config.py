"""Configuration and constants for VoxBridge TTS package.
...
# Available models (keeping original HF repo names since those are the open weights)
...
# `VOXBRIDGE_MODEL_REVISION` environment variable.
#
# HuggingFace repos keep pointing to Supertone/supertonic-* (open weights under OpenRAIL-M).
# The cache_dir values are rebranded for VoxBridge.
MODEL_CONFIGS = {
    "supertonic": {
...
DEFAULT_MODEL_REPO = os.getenv("VOXBRIDGE_MODEL_REPO", MODEL_CONFIGS[DEFAULT_MODEL]["repo"])
# ``VOXBRIDGE_CACHE_DIR`` is intentionally not snapshotted to a module-level
...
MODEL_REVISION_ENV_OVERRIDE = os.getenv("VOXBRIDGE_MODEL_REVISION")
...
def get_model_cache_dir(model_name: str) -> Path:
    ...
    ``$VOXBRIDGE_CACHE_DIR`` overrides the directory for *every* model when
...
def get_model_revision(model_name: str) -> str:
    ...
    Priority: ``VOXBRIDGE_MODEL_REVISION`` env var (if set) → the
...
DEFAULT_INTRA_OP_NUM_THREADS = _parse_env_int("VOXBRIDGE_INTRA_OP_THREADS")
DEFAULT_INTER_OP_NUM_THREADS = _parse_env_int("VOXBRIDGE_INTER_OP_THREADS")
...
LOG_LEVEL = os.getenv("VOXBRIDGE_LOG_LEVEL", "INFO")