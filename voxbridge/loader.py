...
This module handles downloading, loading, and managing VoxBridge TTS models
...
from .core import Style, VoxBridge, UnicodeProcessor
...
def get_cache_dir(model_name: Optional[str] = None) -> Path:
    """Get or create the cache directory for VoxBridge models.
    ...
    Note:
        ...
        (e.g., ``~/.cache/voxbridge3`` for supertonic-3), but can be
        overridden with the ``VOXBRIDGE_CACHE_DIR`` environment variable.
...
def download_model(model_dir: Union[Path, str], model_name: Optional[str] = None) -> None:
    """Download VoxBridge model from HuggingFace Hub.
...
def load_model(
    ...
    """Load the complete VoxBridge TTS model.
    ...
    Returns:
        Initialized VoxBridge TTS engine
    """
    ...
    return VoxBridge(cfgs, text_processor, dp_ort, text_enc_ort, vector_est_ort, vocoder_ort)
