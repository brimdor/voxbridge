...
This module contains the main VoxBridge TTS engine, text processor,
...
class UnicodeProcessor:
    ...
    def _preprocess_text(self, text: str, lang: Optional[str] = None) -> str:
        ...
        Args:
            text: Raw input text
            lang: Language code for multilingual support. VoxBridge-3
                supports 31 ISO codes plus the ``"na"`` fallback for
                unknown languages; VoxBridge-2 supports
                ``en``, ``ko``, ``es``, ``pt``, ``fr``.
        ...
        # Add language tokens for multilingual models (voxbridge-2 / voxbridge-3)
        if lang is not None:
...
class VoxBridge:
    """Core TTS engine for VoxBridge speech synthesis.
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized VoxBridge engine (sample_rate={self.sample_rate}Hz, "
    ...
    def __call__(
        ...
        Args:
            ...
            lang: Language code for multilingual support.
                Required for multilingual models (voxbridge-2 / voxbridge-3).
                See ``AVAILABLE_LANGUAGES`` for the full list. Use ``"na"``
                for unknown / unsupported languages (voxbridge-3 only).
