"""High-level TTS interface for VoxBridge.

This module provides the main TTS class for easy text-to-speech synthesis
with automatic model loading, voice style management, text normalization,
...
class TTS:
    ...
    Args:
        ...
        intra_op_num_threads: Number of threads for intra-op parallelism.
            ...
            Can also be set via VOXBRIDGE_INTRA_OP_THREADS environment variable
        inter_op_num_threads: Number of threads for inter-op parallelism.
            If None (default), ONNX Runtime automatically determines optimal value based on your system.
            Can also be set via VOXBRIDGE_INTER_OP_THREADS environment variable
        normalizer: If True (default), automatically normalize text before synthesis.
            Can also pass a Normalizer instance for custom configuration.
        expressions: If True (default), process expression tags like <laugh>,
    ...
    def __init__(
        ...
        normalizer: Union[bool, "Normalizer"] = True,
        expressions: Union[bool, "ExpressionProcessor"] = True,
    ):
        ...
        Args:
            ...
            inter_op_num_threads: Number of threads for inter-op parallelism.
                If None (default), ONNX Runtime automatically determines optimal value based on your system.
            normalizer: If True, use default Normalizer; if a Normalizer instance, use that;
                if False, disable text normalization.
        ...
        # Set up expression processor
        if isinstance(expressions, ExpressionProcessor):
            self.expression_processor = expressions
        elif expressions:
    ...
    def synthesize(
        ...
        and concatenates them with silence in between. If text normalization
        or expression processing is enabled, they are applied automatically.
        ...
        # Extract expression tags before normalization
        expression_tags = []
        if self.expression_processor:
            text, expression_tags = self.expression_processor.extract(text)
